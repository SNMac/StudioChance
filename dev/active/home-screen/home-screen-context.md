...
- 20-2 (근본 원인 수정): unmount/remount 시 stale `initialScrollOffset` 문제 해결
  - `_controllerForPage`에서 `!hasClients` 감지 → dispose + 재생성 로직 추가
...
### 20-1: 3일 캘린더 날짜별 수직 스크롤 위치 어긋남 — 근본 원인 확정 및 수정

**현상**: 좌우 스와이프로 날짜를 이동할 때 일부 날짜 열의 수직 스크롤 위치가 다른 열과 맞지 않음.

**수정 이력**:
- 19-1 (1차): `isInitialized` 플래그 + postFrameCallback → 여전히 재현
- 20-1 (2차): `isInitialized = true` 순서 변경 + `scheduleInit()` 재귀 → 여전히 재현
- 20-2 (3차, **근본 원인 수정**): unmount/remount 시 stale `initialScrollOffset` 문제 해결 ✅

**근본 원인 (20-2에서 확정)**:

`ScrollController.initialScrollOffset`은 `final` 필드로, 최초 생성 시점에 고정된다. Flutter는 컨트롤러가 새 `ScrollPosition`에 **재연결(re-attach)될 때마다** `createScrollPosition(initialPixels: initialScrollOffset)`을 호출하여 이 stale 값을 사용한다.

**버그 발생 경로**:
```
1. 페이지 98 컨트롤러 생성: initialScrollOffset = Y (당시 _currentVerticalOffset)
   → scheduleInit() 실행 → isInitialized = true

2. 사용자 수직 스크롤 → _currentVerticalOffset = Z

3. 페이지 98이 PageView cache extent 밖으로 나감 → 위젯 unmount
   → ctrl98.hasClients = false
   → ±5 범위 이내이므로 _dayScrollControllers에서 제거 안 됨

4. 사용자 추가 수직 스크롤 → _currentVerticalOffset = W

5. 페이지 98 다시 화면 진입 → itemBuilder(98) 호출
   → _controllerForPage(98): putIfAbsent → 기존 ctrl98 반환 (재생성 안 함)

6. SingleChildScrollView가 ctrl98 재연결(re-attach)
   → Flutter: ctrl98.createScrollPosition(initialPixels: Y) ← stale!
   → notifyListeners() 발생

7. 리스너 실행: isInitialized = true → offset Y ≠ W
   → _currentVerticalOffset = Y ← stale로 덮어씀!
   → _syncAllScrollControllers(Y) → 전체 열이 Y로 snap
```

**핵심 포인트**:
- `_evictDistantControllers`는 ±5 범위 밖만 제거 → 범위 안 페이지는 unmount돼도 컨트롤러가 맵에 남음
- PageView의 cache extent(기본 250px)는 ±5 범위보다 훨씬 좁음 → 그 사이 페이지들이 자주 unmount/remount
- `initialScrollOffset`을 갱신할 방법이 없음 (`final` 필드)

**해결 방법 (20-2, 구현 완료)**:
`_controllerForPage`에서 `!hasClients`인 기존 컨트롤러를 감지 → dispose 후 현재 offset으로 재생성

```dart
ScrollController _controllerForPage(int page) {
  // 페이지가 unmount되었다가 remount될 때: 기존 컨트롤러의 initialScrollOffset이 stale
  ...
  return _dayScrollControllers.putIfAbsent(page, () {
    // 새 컨트롤러: 현재 시점의 정확한 _currentVerticalOffset 사용
    ...
  });
}
```
...
**안전성 검증**:
- 현재 마운트된 페이지 재진입: `hasClients = true` → 재생성 안 함 ✓
- dispose된 컨트롤러: `hasClients = false` → 재생성, `scheduleInit()` 재등록 ✓
- 구 `scheduleInit()` 잔류: `containsValue(old_ctrl)` = false → 자동 중단 ✓
