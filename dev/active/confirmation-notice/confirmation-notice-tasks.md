...
- [x] **4-2** 앱 실행 → 예약 상세 모달 → '확정 안내문' 탭 → 화면 진입 확인
- [ ] **4-3** 안내문 텍스트 플레이스홀더 치환 확인
- [x] **4-4** 이용 장소: `address`, `addressDetail`, `addressGuide` 공백 연결 확인
- [ ] **4-5** `confirmationNotes` 있을 때 📌 섹션 표시 확인
- [x] **4-6** `confirmationNotes` null 시 📌 섹션 생략 확인
- [x] **4-7** 복사하기 → 클립보드 + SnackBar 확인
- [x] **4-8** 공유하기 → 시스템 공유 시트 표시 확인
- [x] **4-9** 뒤로가기 → 예약 상세 모달로 복귀 확인
