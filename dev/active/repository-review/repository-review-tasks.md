...
- [ ] **[BUG-1]** `reservation_repository_impl.dart:107` `StoreColor.blue` → `StoreColor.red` 수정
...
- [ ] **[FIRESTORE-2]** `fetchOrCreateUser` 기존 유저 write 2회 → 1회로 합산
...
- [ ] **[FIRESTORE-1]** `updateReservation` — `createdAt` 필드 덮어쓰기 방지
  - 파일: `lib/data/repositories/reservation_repository_impl.dart`
...
- [ ] **[PERF-3]** `fetchOrCreateUser` — `lastLoginAt` 기존 유저 로그인 시 갱신
  - 파일: `lib/data/repositories/user_repository_impl.dart`
